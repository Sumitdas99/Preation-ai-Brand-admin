...
export function PreFlightErrorScreen({ error, onRetry }: Props) {
...
function errorPresentation(error: Error) {
  ...
  if (error instanceof ForbiddenError) {
    return {
      icon: Lock,
      title: "You don't have access to this pre-flight run",
      description:
  ...
  if (error instanceof NotFoundError) {
    return {
      icon: Search,
      title: "Pre-flight run not found",
      description:
        "This pre-flight run may have been deleted or the link is incorrect.",
    };
  }
