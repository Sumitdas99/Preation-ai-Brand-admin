...
const COLLAPSED_KEY = "praetion.devLegalDashboardPanel.collapsed";
const POSITION_KEY = "praetion.devLegalDashboardPanel.position";
...
export function DevLegalDashboardPanel({ role, onRoleChange }: Props) {
  ...
  if (collapsed) {
    return (
      <div ref={containerRef} style={containerStyle} className="fixed z-50">
        <button
          type="button"
          aria-label="Open dashboard dev panel"
  ...
  return (
    ...
    >
      <button
        type="button"
        aria-label="Drag dashboard dev panel or click to collapse"
        onPointerDown={onHandlePointerDown}
        onPointerMove={onHandlePointerMove}
        onPointerUp={onHandlePointerUp}
        onPointerCancel={onHandlePointerUp}
      ...
      <div className="flex min-w-[10rem] flex-col border-l border-white/10 pl-3 leading-tight">
        ...
        <span className="text-xs font-medium">
          {role === "Legal" ? "Legal Approver queue" : "My Submissions"}
        </span>
      </div>
