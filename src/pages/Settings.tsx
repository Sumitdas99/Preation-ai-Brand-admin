...
export default function Settings() {
  ...
  return (
    <div className="flex flex-col md:flex-row h-full bg-background gap-4 md:gap-6 p-4 md:p-6 animate-fade-in">
      {/* Left Sidebar */}
      <aside className="w-full md:w-64 border border-border rounded-lg bg-card p-4 shrink-0">
        <h2 className="mb-4 px-3 text-lg font-semibold text-foreground">Settings</h2>
        <nav className="flex md:flex-col flex-wrap gap-1.5 md:space-y-1">
          {getSettingsNav(userRole).map((item) => (
            <button
              key={item.id}
              onClick={() => setActiveTab(item.id)}
              className={cn(
                "rounded-lg px-3 py-2.5 text-left text-sm font-medium transition-colors flex-1 md:flex-initial text-center md:text-left",
                activeTab === item.id
      ...
      {/* Main Content */}
      <main className="flex-1 overflow-y-auto p-0 md:p-6 mt-2 md:mt-0">
        {activeTab === "profile" && (
          <Card>
            <CardHeader>
