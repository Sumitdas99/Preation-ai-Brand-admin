  ...
  return (
    <ViewerRoleContext.Provider value={viewerRole}>
      <PageBody
        ...
        evidencePackId={preflight.evidence_pack?.evidence_pack_id}
...
function PageBody(props: PageBodyProps) {
  ...
  const actionsValue = usePreFlightActionsController({
    ...
    evidencePackId: props.evidencePackId,
  ...
  return (
    <PreFlightActionsContext.Provider value={actionsValue}>
      <div className="flex h-full min-h-0 flex-col overflow-hidden bg-background">
        <TopBar data={props.data.topBar} onMenuClick={() => setSidebarOpen((v) => !v)} />
        <StageBanner data={props.data.banner} />

        <div className="relative flex flex-1 overflow-hidden">
          <div
            className={`absolute inset-0 z-30 bg-black/60 transition-opacity duration-300 ease-in-out md:hidden ${sidebarOpen ? "opacity-100" : "pointer-events-none opacity-0"}`}
            onClick={() => setSidebarOpen(false)}
            aria-hidden
          />
          <div
            className={`absolute inset-y-0 left-0 z-40 w-64 max-w-[80vw] shadow-xl transition-transform duration-300 ease-in-out md:hidden ${sidebarOpen ? "translate-x-0" : "-translate-x-full"}`}
          >
            <PreFlightSidebar data={props.data} className="h-full w-full" />
          </div>

          <PreFlightSidebar data={props.data} className="hidden md:flex" />
          <MainArea
            data={props.data}
